...
test.describe('Concurrent Execution', () => {
  ...
  test('should start multiple executions concurrently and view logs', async ({ page }) => {
    ...
    if (await secondExecution.count() > 0) {
      ...
      await expect(page.getByRole('heading', { name: 'create_hello-world' })).toBeVisible();
      await expect(page.locator('div[role="tabpanel"]').filter({
    ...
    for (let i = 0; i < completedCount; i++) {
      ...
      const execution = historyDialog.locator('li').filter({
        hasText: /Status: (Completed|Failed|Canceled)/
      }).first();
  ...
  test('should persist execution history across page reloads', async ({ page }) => {
    // Find the Hello world Digital Twin card
    const helloWorldCard = page
      .locator('.MuiPaper-root:has-text("Hello world")')
      .first();
    ...
    const preReloadHistoryButton = helloWorldCard.getByRole('button', { name: 'History' }).first();
    ...
    // Click the History button
    const postReloadHistoryButton = helloWorldCard.getByRole('button', { name: 'History' }).first();
